...
const MenuItemModal = ({ item, onClose }: MenuItemModalProps) => {
  ...
  const [selectedVariants, setSelectedVariants] = useState<Record<string, { name: string; additionalPrice: number }>>(() => {
  ...
  const variantCost = Object.values(selectedVariants).reduce((s, v) => s + v.additionalPrice, 0);
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed inset-4 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:w-full md:max-w-lg z-50 bg-card rounded-2xl shadow-elevated overflow-hidden flex flex-col max-h-[90vh]"
      >
        {/* Image */}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs font-medium">{item.averageRating} ({item.reviewCount})</span>
            </div>
          </div>
          ...
          {item.dietaryTags.length > 0 && (
            <div className="flex gap-1.5 flex-wrap mb-4">
              {item.dietaryTags.map((tag) => (
                <span key={tag} className="px-2 py-0.5 bg-olive/10 text-olive rounded text-xs capitalize">{tag}</span>
              ))}
            </div>
          )}
          ...
          {item.variants?.map((group) => (
            <div key={group.groupName} className="mb-4">
              <label className="text-sm font-semibold mb-2 block">{group.groupName}</label>
              <div className="flex flex-col gap-2">
                {group.options.map((option) => (
                  <button
                    key={option.name}
                    onClick={() => setSelectedVariants((prev) => ({ ...prev, [group.groupName]: option }))}
                  ...
                  >
                    ...
                    <span className="text-muted-foreground">
                      {option.additionalPrice > 0 ? `+${formatPrice(option.additionalPrice)}` : 'Included'}
                    </span>
                  </button>
                ))}
              </div>
        ...
        {/* Footer */}
        <div className="border-t border-border p-6 flex items-center gap-4 shrink-0">
